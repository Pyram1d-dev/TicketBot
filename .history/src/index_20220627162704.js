...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            let stud = client.emojis.cache.get('991083932147646544');
            msg.react('✅').then(() => {msg.react('❌').then(() => msg.react(stud))}).catch(console.error);
            const filter = (reaction, user) => ['✅', '❌', stud.name].includes(reaction.emoji.name) && !user.bot;
            // 1,200,000 ms = 20 mins
            msg.awaitReactions({filter, time: 1200000})
            .then(collected => {
                if (endedOperation) return;

                const reaction = collected.first();

                if ((['✅', '❌'].includes(reaction.emoji.name) && reaction.users.cache.count < 3) && reaction.emoji != stud)
                    return;
                ...
                switch (reaction.emoji.name)
                {
                    case '✅' | stud.name:
                        console.log("Yeah :)");
                        let forceAccepted = reaction.emoji === stud;
                        resultEmbed.setColor('#3dfc03')
                            .setTitle('Announcement Request')
                            .setAuthor({name: user.username, iconURL: user.avatarURL(), })
                            .setDescription(`\"${desc}\"\n\n\`This request was${forceAccepted ? " force" : ""} accepted ${forceAccepted ? `by ${reaction.users.cache[0].username}` : ""}.\``)
                            .setTimestamp(timestamp);
                            await user.send(`Your request to announce \"${desc}\" has been approved!`);
                        break;
                    default:
                        console.log("No >:(");
                        ...
                        .setTimestamp(timestamp);
                        await user.send(`Your request to announce \"${desc}\" was denied.`);
                }

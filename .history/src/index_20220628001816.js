...
function setEmbedStyle(embed, user, desc, timestamp, style, forceAccepted = false, reactor = null)
{
    let styleData = embedStyles[style];
    let resultText = styleData.resultText.replace('{username}', user.username);
    if (forceAccepted)
        resultText = `This request was force accepted by ${reactor.username}.`
            ...
            collector.on('collect', async (reaction, reactor) => {
                ...
                switch (reaction.emoji.name)
                {
                    ...
                    case '✅':
                        ...
                        setEmbedStyle(resultEmbed, user, desc, timestamp, "approved", true, reactor);
                    ...
                    case 'youshould':
                        ...
                        resultEmbed.setColor('#f70c0c')
                            .setTitle('Announcement Request')
                            .setAuthor({name: user.username, iconURL: user.avatarURL()})
                            .setDescription(`\"${desc}\"\n\n\`This request was stupid and we elected to ban ${user.username} from requesting.\``)
                            .setTimestamp(timestamp);

...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        ...
        case 'cancelrequest':
            ...
            console.log(usersRequested[user.id]['timestamp'])
            if (Date.now() - usersRequested[user.id]['timestamp'] < 10000)
            {
                await interaction.reply({content: `You must wait at least ${Math.round((Date.now() - usersRequested[user.id]['timestamp']) / 1000 * 100) / 100} more seconds before cancelling your request.`, ephemeral: true}).catch(console.error);
                break;
            }
            usersRequested[user.id]['cancel']();
            usersRequested[user.id] = null;

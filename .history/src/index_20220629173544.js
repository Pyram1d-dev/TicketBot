    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            collector.on('collect', async (reaction, reactor) => {
                ...
                if (modCooldowns.has(reactor.id) && Date.now() - modCooldowns.get(reactor.id) <= 24*60*60*1000)
                {
                    let txt = "seconds";
                    let time = (Date.now() - modCooldowns.get(reactor.id)) / 1000
                    if (time / 60 >= 1)
                    {
                        txt = "minutes";
                        time /= 60;
                    ...
                    reactor.message(`You cannot force accept a request for another ${truncateFloat(time, 1)} ${txt}!`);

...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            {
                if ((Date.now() - cooldowns[user.id]['startTime']) / 1000 > cooldowns[user.id]['banTime'])
            }
            ...
            collector.on('collect', async (reaction, reactor) => {
                ...
                switch (reaction.emoji.name)
                {
                    ...
                    case 'youshould':
                        console.log('stfu');
                        user.send(`Your request was fucking stupid and you are unable to message for 5 minutes because of it. #ripbozo`);
                        cooldowns[user.id] = {startTime: Date.now(), banTime: 300};
                        break;
                    default:
                        console.log("No >:(");

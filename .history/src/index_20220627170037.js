...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            const filter = (reaction, user) => ['✅', '❌', 'gamerstud'].includes(reaction.emoji.name) && !user.bot;
            ...
            collector.on('collect', (reaction, reactor) => {
                ...
                if (!(['✅', '❌'].includes(reaction.emoji.name) && reactionCount >= 3) && reaction.emoji.name != 'gamerstud')
                {
                ...
                switch (reaction.emoji.name)
                {
                    ...
                    case '✅':
                        ...
                        resultEmbed.setColor('#3dfc03')
                            .setTitle('Announcement Request')
                            .setAuthor({name: user.username, iconURL: user.avatarURL(), })
                            .setDescription(`\"${desc}\"\n\n\`This request was${forceAccepted ? " force" : ""} accepted${forceAccepted ? ` by ${reactor.usename}` : ""}.\``)
                            .setTimestamp(timestamp);

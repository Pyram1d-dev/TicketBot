    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            if (user.id in cooldowns)
            {
                ...
                {
                    await interaction.reply({content: `You are still banned from requesting tickets. Wait ${truncateFloat(cooldowns[user.id]['banTime'] - timeElapsed, 1)} seconds.`, ephemeral: true}).catch(console.error);
                    break;
                }
            ...
            collector.on('collect', async (reaction, reactor) => {
                ...
                switch (reaction.emoji.name)
                {
                    ...
                    case 'youshould':
                        console.log('stfu >>:((');
                        let banTime = 600;
                        user.send(`Your request was fucking stupid and you are unable to message for ${banTime / 60} minutes because of it. #ripbozo`);

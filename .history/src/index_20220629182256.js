    ...
    switch (interaction.commandName)
    {
        case 'request':
            ...
            collector.on('collect', async (reaction, reactor) => {
                ...
                if (modCooldowns.has(reactor.id) && Date.now() - modCooldowns.get(reactor.id) <= 24*60*60*1000 && forceAccepted)
                {
                    let txt = "seconds";
                    let time = (24*60*60*1000 - (Date.now() - modCooldowns.get(reactor.id))) / 1000;
                    if (time / 60 >= 1)
                    {
                        txt = "minutes";
                        time /= 60;

...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        ...
        case 'cancelrequest':
            ...
            {
                await interaction.reply({content: `You must wait at least ${10 - (Math.round((Date.now() - usersRequested[user.id]['timestamp']) / 100) / 10)} more seconds before cancelling your request.`, ephemeral: true}).catch(console.error);
                break;
            }
            usersRequested[user.id]['cancel']();
            usersRequested[user.id] = null;

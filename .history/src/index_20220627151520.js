...
client.on('interactionCreate', async interaction => {
    ...
    let channel = await fetchChannel(interaction.member.guild, channelIDs.moderator);
    let desc = interaction.options.getString('description');
    let user = interaction.user;
    switch (interaction.commandName)
    {
        case 'request':
            await interaction.reply({content: 'Permission has been requested, please wait...', ephemeral: true}).catch(console.error);
            const coolEmbed = new MessageEmbed()
                ...
                .setDescription(`\"${desc}\"`)
                .setTimestamp();
            await channel.send({embeds: [coolEmbed]}).catch(console.error);
            break;
    }
});

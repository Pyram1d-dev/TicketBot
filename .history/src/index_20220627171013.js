...
client.on('interactionCreate', async interaction => {
    ...
    switch (interaction.commandName)
    {
        ...
        case 'cancelrequest':
            if (!(user.id in usersRequested))
            {
                await interaction.reply({text: 'You don\'t have a pending request!', ephemeral: true}).catch(console.error);
                break;
            }
            if (Date.now() - usersRequested[user.id]['timestamp'] < 8000)
            {
                await interaction.reply({text: `You must wait at least ${(Date.now() - usersRequested[user.id]['timestamp']) / 1000} more seconds before cancelling your request.`, ephemeral: true}).catch(console.error);
                break;
            }
            usersRequested[user.id]['cancel']();
            usersRequested[user.id] = null;
            await interaction.reply({text: `Your request has been cancelled.`, ephemeral: true}).catch(console.error);
    }
});
